...
const activitiesSparkData = [
  { value: 120 }, { value: 135 }, { value: 150 }, { value: 145 },
  { value: 165 }, { value: 180 }, { value: 178 },
]
...
export default function CompletedActivitiesCard() {
  ...
  const count = dashboard?.status?.completed_activities || 0
      ...
      <Card className="group border border-gray-200 dark:border-neutral-700/50 rounded-xl overflow-hidden bg-white dark:bg-neutral-900 hover:shadow-lg hover:shadow-black/5 dark:hover:shadow-black/20 transition-all duration-300 hover:border-gray-300 dark:hover:border-neutral-600">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-amber-500 via-amber-600 to-amber-700 text-white shadow-lg shadow-amber-500/25 dark:shadow-amber-500/15 transition-transform duration-200 group-hover:scale-110">
              <Settings className="h-4 w-4" />
            </div>
            <span className="font-medium text-foreground">Finished Activities</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex items-baseline gap-3">
            ...
            <TrendBadge value="+8.2%" positive={true} />
          </div>
          <div className="h-20">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={activitiesSparkData}>
                <Line
                  type="linear"
                  dataKey="value"
                  stroke="#f59e0b"
